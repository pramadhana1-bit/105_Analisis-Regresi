...
```{r}
library(readxl)
dt <- read_xlsx("C:\\Users\\Roshan\\OneDrive\\Documents\\Kuliah\\ANREG\\Data Timbunan Sampah.xlsx")
dt
```

```{r}
y <- dt$"Timbunan Sampah / Tahun (TON)"
x <- dt$"X2 (PDRB per Kapita)"
n <- length(dt)
```
...
$$JKT = JKR + JKG$$
Nilai penguraian keragaman ini dapat diamati pula menggunakan fungsi anova pada bagian **Sum Sq (Jumlah Kuadrat)** seperti yang ditampilkan di bawah. Pada output sintaks di bawah, JKR model bernilai 6.4966e+14 dan JKG model bernilai 1.0083e+18

```{r}
(anova.model <- anova(model))
```

Dari tabel ANOVA dan penguraian keragaman, dapat diduga pula ragam galat/error dari model yang kita peroleh. Secara umum, error dari model dapat diduga oleh nilai Kuadrat Tengah Galat (KTG) sehingga berdasarkan output sintaks di atas, diketahui bahwa ragam galat dari model bernilai 2.880814e+16. Hubungan antara keduanya dapat dituliskan sebagai berikut.
...
Sebelumnya, melalui fungsi `lm` dan perhitungan manual yang sudah dibahas pada pertemuan sebelumnya, kita sudah mendapatkan nilai dugaan parameter β0 dan β1 sebesar 92973138 dan 116.5663. Kedua nilai dugaan parameter tersebut dapat kita definisikan sebagai berikut.
...
Sehigga dapat disimpulkan bahwa pada taraf signifikansi 5%, selang kepercayaan untuk parameter $β_0$ adalah [ -82139696 ,  268085973 ] dan untuk parameter $β_1$ adalah [ -1782.787 ,  2015.92 ]
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan rataan nilai $Y$ ketika nilai x=10 adalah 92974304. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai dugaan rataan $Y$ ketika nilai x=10 berada dalam selang -4354505 hingga 190303113.
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan nilai individu Y ketika nilai x=10 adalah 1 92974304. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai amatan individu Y ketika nilai x=10 berada dalam selang -265077372 hingga 451025980.